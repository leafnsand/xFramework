...
#define CRC32(c, b)  (crc_table[((int)(c) ^ (b)) & 0xff] ^ ((c) >> 8))
        unsigned const char* buf = reinterpret_cast<unsigned const char*>(data);
        if (buf == nullptr)
        {
            m_value = 0;
        }
        else
        {
            ...
            m_value = crc ^ 0xffffffffL;     /* (instead of ~c for 64-bit machines) */
        }
#undef CRC32
    }
    ...
    // Crc32 - Add
    //=========================================================================
    void Crc32::Add(const char* str)
    {
        if (str)
        {
            size_t len = strlen(str);
            u32 crc = Crc32(str, len, true);
            Combine(crc, len);
        }
    }
    ...
    {
        u32 crc = Crc32(data, size, forceLowerCase);
        Combine(crc, size);
    }

    inline u32 MatrixTimes(u32* mat, u32 vec)
    {
        u32 sum;
        sum = 0;
        while (vec)
    ...
    // Crc32 - Combine
    //=========================================================================
    void Crc32::Combine(u32 crc, size_t len)
    {
